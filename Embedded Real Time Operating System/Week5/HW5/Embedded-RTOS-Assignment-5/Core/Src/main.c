...
int main(void)
{
  ...
  osMessageQDef(Queue01, 5, uint32_t);
  ...
  osThreadDef(Task1, SenderTask, osPriorityNormal, 0, 128);
  Task1Handle = osThreadCreate(osThread(Task1), NULL);

  /* definition and creation of Task2 */
  osThreadDef(Task2, ReceiverTask, osPriorityNormal, 0, 128);
...
void SenderTask(void const * argument)
{
  ...
  for(;;)
  {
	osMessagePut(Queue01Handle, count, osWaitForever);
	count++;
    osDelay(2000);
  }
  /* USER CODE END 5 */
