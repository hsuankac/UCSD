...
/* USER CODE BEGIN PV */
uint8_t swInterruptFlag = 0;
volatile uint8_t tx_flag = 0;
volatile uint8_t rx_flag = 0;
volatile uint8_t alarm_flag = 0;
static volatile uint8_t tim3Count = 0;
char in[1] = {0};
/* USER CODE END PV */
  ...
  while (1)
  {
	...
	if(rx_flag)
	{
		...
		case 'g':
		{
			...
			myDelay2(1000);
			logMsg(&huart1, "Delay 1 second complete using systick complete\r\n");
			break;
		}
		case 'b':
		{
			...
			myDelay1(1000);
			logMsg(&huart1, "Delay 1 second complete using TIM2 complete\r\n");
			break;
		}
		case 's':
		{
		...
		case 't':
		{
			...
			snprintf(buf, sizeof(buf), "Total counted timer3 event = %d\r\n", tim3Count);
			logMsg(&huart1, buf);
			tim3Count = 0;
...
void myDelay1(uint32_t val) {
	...
	htim2.Instance->ARR = val - 1;
	htim2.Instance->CR1 = 1;
	htim2.Instance->CNT = 0;
	while (!(htim2.Instance->SR & TIM_SR_UIF));
...
void myDelay2(uint32_t val) {
	...
	while ((HAL_GetTick() - tickstart) < val);
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim == &htim3)
	{
		char buf[100];
		tim3Count++;
		snprintf(buf, sizeof(buf), "Counted timer3 event = %d\r\n", tim3Count);
		logMsg(&huart1, buf);
	}
